...
async function loadAlerts() {
  ...
  try {
    ...
    $("#updated-at").textContent = ui("API 离线", "API offline");
  } finally {
...
async function loadCaptureStatus() {
  try {
    ...
    if (s.error || (s.errors && s.errors.length)) {
      $("#capture-btn").textContent = ui("抓取异常", "Capture Error");
      $("#capture-btn").title = s.error || s.errors[0];
    } else if (s.ts) {
      $("#capture-btn").textContent = ui("自动抓取中", "Auto Capture");
      $("#capture-btn").title = ui(`已抓取 ${s.fetched || 0} 条，新增 ${s.captured || 0} 条`, `Fetched ${s.fetched || 0}, new ${s.captured || 0}`);
    }
  } catch {}
}
...
$("#capture-btn").addEventListener("click", async () => {
  $("#capture-btn").textContent = ui("抓取中", "Capturing");
  try {
    await fetch("/api/capture", { method: "POST", headers: { "x-ghost-action": "1" } });
    await loadAlerts();
    await loadCaptureStatus();
  } catch {
    $("#capture-btn").textContent = ui("抓取失败", "Capture Failed");
  }
});
...
$("#analyze-form").addEventListener("submit", async (e) => {
  ...
  try {
    const res = await fetch("/api/analyze", {
      method: "POST",
      headers: { "content-type": "application/json", "x-ghost-action": "1" },
