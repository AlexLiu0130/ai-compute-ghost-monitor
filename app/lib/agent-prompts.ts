export const AGENT_PROMPT_VERSION = "ghost-harness-v5";

export const GHOST_TYPES = [
  "compute_overcapacity", "capex_roi_doubt", "order_inventory_weakness", "hbm_shortage",
  "capacity_flood", "demand_order_strength", "data_center_delay", "financing_stress", "capital_markets_memory",
...
export function critiqueMessages(row: Record<string, unknown>, analysis: unknown, _preliminary: unknown) {
  return [
    { role: "system", content: SYSTEM },
    { role: "user", content: `Act as a skeptical reviewer. Return JSON matching this schema: ${JSON.stringify(CRITIQUE_SCHEMA)}. confidence is an integer: 0=unsupported, 1=weak, 2=materially supported, 3=explicitly supported. Confirm only evidence supported by the article. Reject or downgrade unsupported, contradictory, stale, generic, holdings, price-target, or listicle claims. corrections may contain event_type and strength only when supported. Put every ticker without an explicit causal path, or whose proposed direction contradicts the supplied evidence, in unsupported_tickers. Do not calculate, propose, or output any final score or probability.\n<untrusted_article_json>\n${articleInput(row)}\n</untrusted_article_json>\n<untrusted_analysis_json>\n${JSON.stringify(analysis)}\n</untrusted_analysis_json>` },
  ];
}
